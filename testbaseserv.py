import socket, select, sys, _thread, subprocess, time
...
def runTrackerServer():
    ...
    while True:
        ...
        if connection not in trackserv_conn_list:
            ...
            spawnRTKLIB(input_port, output_port, tracker_id)
            time.sleep(3)
...
def spawnRTKLIB(input_port, output_port, tracker_id):
    new_config = config
    #set input
    ...
    while True:
        ...
        if ready[0]:
            try:
                ...
                eprint(tracker_id + ': socket error: ' + str(e), RTKOUTSERV)
                break
            except:
            ...
            if (data != b''):
                ...
                if (data[0] != 37):
                    print("SENDING DATA")
                    parsed_data = parseDataFromRTKLIB(tracker_id, data)
                    for ext_conn in aggserv_conn_list:
                        print("SENDING DATA LOOP")
                        ext_conn.sendall(data)
            else:
                eprint(tracker_id + ': socket closed', RTKOUTSERV)
                break
    connection.close()

def parseDataFromRTKLIB(tracker_id, data):
    ...
    e = "{:14.4f}".format(float(data_parts[2]))
    n = "{:14.4f}".format(float(data_parts[3]))
    return (str(tracker_id) + "," + str(GPST_week) + "," + str(GPST_seconds) + "," + str(e) + "," + str(n)).encode('UTF-8')


with open(CONFIG_FILE, 'r') as myfile:
    config = myfile.read()
_thread.start_new_thread(runAggregatorServer, ())
_thread.start_new_thread(handleTrackerConnections, ())
runTrackerServer()
